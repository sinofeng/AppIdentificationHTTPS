...
def model_fn(features, labels, mode, params):
    ...
    net1=tf.layers.dense(inputs=net1, units=20)
    ...
    net2 = tf.layers.dense(inputs=output[:,-1,:], name='layer_rnn_fc_1',
                          units=128, activation=tf.nn.relu)
    net2 = tf.layers.dense(inputs=net2, units=20)
    ...
    net3 = tf.layers.dropout(inputs=net3,rate=0.2)
    net3 = tf.layers.dense(inputs=net3, units=20)
    ...
    net4 = tf.layers.dense(inputs=net4, name='layer_length_fc1',
                          units=128, activation=tf.nn.relu)
    net4  = tf.layers.dense(inputs=net4, units=20)
    if choose == "model_cnn1d":
        net = net1
        net = tf.layers.dense(inputs=net, units=20, activation=tf.nn.relu)
    if choose == "model_cnn1d_cnn1d":
        net = tf.concat([net1,net4],1)
        net = tf.layers.dense(inputs=net, units=40, activation=tf.nn.relu)
    ...
    if choose == "model_cnn1d_cnn1d_rnn":
        ...
        net = tf.layers.dense(inputs=net, units=60, activation=tf.nn.relu)
...
alphabet=softwares=["Baidu Map",
                    ...
                    "Tencent",
                    "Alipay"]
figures.plot_confusion_matrix(y_true, predicts,alphabet, "./unb_%dx%d_"% (pkt_counts, pkt_size) + choose)
