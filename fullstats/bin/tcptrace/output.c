    ...
    tcp_pair *ptp)
{
	...
	*/ 
	...
	if(print_denis) {
		StatLineP("req 1323 ws/ts","","%s","NA","NA");
	}
	}
	...
	if(print_denis) {
	StatLineP("adv wind scale","","%s","NA","NA");
	}
	}
	...
	if(print_denis) {
	StatLineP("req sack","","%s","NA","NA");
	StatLineP("sacks sent","", "%s", "NA", "NA");
	}
	}
    ...
    StatLineF("data xmit time","secs","%7.6f",
	      etime_data1 / 1000000.0,
	      etime_data2 / 1000000.0);
    StatLineP("idletime max","s","%s",
	      ZERO_TIME(&pab->last_time)?"NA":
	      (snprintf(bufl,sizeof(bufl),"%8.6f",(double)pab->idle_max/1000000.0),bufl),
	      ZERO_TIME(&pba->last_time)?"NA":
	      (snprintf(bufr,sizeof(bufr),"%8.6f",(double)pba->idle_max/1000000.0),bufr));

    if (((pab->num_hardware_dups != 0) || (pba->num_hardware_dups != 0)
|| csv || tsv || (sv != NULL))  && !print_denis) {
	StatLineI("hardware dups","segs",
		  pab->num_hardware_dups, pba->num_hardware_dups);

        if(!(csv || tsv || (sv != NULL)) && !print_denis)       
	  fprintf(stdout,
	...
	StatLineF("RTT full_sz min","s","%8.6f",
		  (double)pab->rtt_full_min/1000000.0,
		  (double)pba->rtt_full_min/1000000.0);
	StatLineF("RTT full_sz max","s","%8.6f",
		  (double)pab->rtt_full_max/1000000.0,
		  (double)pba->rtt_full_max/1000000.0);
	StatLineF("RTT full_sz avg","s","%8.6f",
		  Average(pab->rtt_full_sum, pab->rtt_full_count) / 1000000.0,
		  Average(pba->rtt_full_sum, pba->rtt_full_count) / 1000000.0);
	StatLineF("RTT full_sz stdev","s","%8.6f",
	...
	if ((pab->rtt_amback || pba->rtt_amback || csv || tsv || (sv != NULL)) && !print_denis) {
	   if(!(csv || tsv || (sv != NULL)))
	     fprintf(stdout, "\
...
");
	    ...
	    StatLineF("RTT min (last)","s","%8.6f",
		      (double)pab->rtt_min_last/1000000.0,
		      (double)pba->rtt_min_last/1000000.0);
	    StatLineF("RTT max (last)","s","%8.6f",
		      (double)pab->rtt_max_last/1000000.0,
		      (double)pba->rtt_max_last/1000000.0);
	    StatLineF("RTT avg (last)","s","%8.6f",
		      Average(pab->rtt_sum_last, pab->rtt_count_last) / 1000000.0,
		      Average(pba->rtt_sum_last, pba->rtt_count_last) / 1000000.0);
	    StatLineF("RTT sdv (last)","s","%8.6f",
