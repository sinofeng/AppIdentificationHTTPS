...
static char const GCC_UNUSED rcsid[] =
    "@(#)$Header: /usr/local/cvs/tcptrace/trace.c,v 5.69 2003/11/19 14:38:06 sdo Exp $";
	...
	* to see the connection node position */
	  ...
	  if (!run_continuously) { 
		...
		printf("SEQ_LESSTHAN init syn: %d\n", 
	     ...
	     if (/* rule 1 */
//(4*60)) - Using nonreal_live_conn_interval instead of the 4 mins heuristic
 		 (!print_denis &&
		 (elapsed(ptp->last_time,current_time)/1000000 > nonreal_live_conn_interval))
		 || /* rule 2 */
		 ((SYN_SET(ptcp)) && 
		  (((thisdir->fin_count >= 1) ||
    ...
    tcb *tcb)
{
	...
	if ((pshd->hwdup_seq == seq) && (pshd->hwdup_id == id) &&
	    ...
	    if (!print_denis && warn_printhwdups) {
